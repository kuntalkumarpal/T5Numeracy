...
class DecodingDataset(Dataset):
    def __init__(self, data_dir, type_path, max_len, random_seed, data_size, representation):
        ...
        self.target_column = ""
        if representation == "split_digit":
            self.target_column = "split_label"  # Evaluate - calculate F1, ACC
...
#         {'text': 'What is one thousand four hundred forty nine ?', 'magnitude': 4, 'label': '1449', 'split_label': '1 4 4 9'}, 
#         {'text': 'What is six hundred sixty seven ?', 'magnitude': 3, 'label': '667', 'split_label': '6 6 7'}
...
if __name__ == "__main__":
    ...
    train_data = DecodingDataset( data_dir = args.data_dir, 
                                  type_path="train", 
                                  max_len=args.max_seq_length,
                                  random_seed=args.seed,
                                data_size=args.data_size,
                                representation=args.representation)
    val_data = DecodingDataset( data_dir = args.data_dir, 
                                  type_path="val", 
                                  max_len=args.max_seq_length,
                                  random_seed=args.seed,
                                data_size=args.data_size,
                                representation=args.representation)
    test_data = DecodingDataset( data_dir = args.data_dir, 
                                  type_path="test", 
                                  max_len=args.max_seq_length,
                                  random_seed=args.seed,
                                data_size=args.data_size,
                                representation=args.representation)
    expol_test_data = DecodingDataset( data_dir = args.data_dir, 
